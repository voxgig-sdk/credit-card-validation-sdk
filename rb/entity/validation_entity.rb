...
class ValidationEntity
  ...
  def data_set(args)
    if args
      @_data = CreditCardValidationHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = CreditCardValidationHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [ValidationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Validation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Validation, Hash] the loaded Validation; raises CreditCardValidationError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = CreditCardValidationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
