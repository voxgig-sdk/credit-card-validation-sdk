# CreditCardValidation Ruby SDK



The Ruby SDK for the CreditCardValidation API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Validation` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
begin
  ...
  validation = client.Validation.load({ "cc" => "example_cc" })
  puts validation
rescue => err
...
```ruby
begin
  validation = client.Validation.load({ "cc" => "example" })
rescue => err
  warn "load failed: #{err}"
end
```
...
```ruby
client = CreditCardValidationSDK.test(testopts, sdkopts)
```
...
raise a `CreditCardValidationError` (a `StandardError` subclass), so wrap
...
```ruby
# load returns the ENTITY — call data_get for the Validation record (raises on error).
validation = client.Validation.load({ "cc" => "cc" })
```
...
```
rb/
├── CreditCardValidation_sdk.rb       -- Main SDK module
...
The main module (`CreditCardValidation_sdk`) exports the SDK class
