...
require_relative 'CreditCardValidation_types'


class CreditCardValidationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = CreditCardValidationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CreditCardValidationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, CreditCardValidationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    CreditCardValidationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = CreditCardValidationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = CreditCardValidationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = CreditCardValidationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => CreditCardValidationError.new(
        "#{op}_allow",
        "CreditCardValidationSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue CreditCardValidationError => err
    ...
    if fetched.is_a?(Hash)
      status = CreditCardValidationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CreditCardValidationError.new(
        "graphql_error", "CreditCardValidationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = CreditCardValidationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
