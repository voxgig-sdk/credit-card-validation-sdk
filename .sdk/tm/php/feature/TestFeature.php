...
// CreditCardValidation SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class CreditCardValidationTestFeature extends CreditCardValidationBaseFeature
{
    ...
    public function init(CreditCardValidationContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (CreditCardValidationContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = CreditCardValidationParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (CreditCardValidationContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(CreditCardValidationContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = CreditCardValidationParam::call($ctx, $k);
