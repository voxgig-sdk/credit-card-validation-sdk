...
// CreditCardValidation SDK retry feature
...
class CreditCardValidationRetryFeature extends CreditCardValidationBaseFeature
{
    ...
    public function init(CreditCardValidationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CreditCardValidationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CreditCardValidationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CreditCardValidationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
