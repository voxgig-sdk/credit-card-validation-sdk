...
module CreditCardValidationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CreditCardValidationResponse.new({})
    ...
    if fetch_err
      ...
      response = CreditCardValidationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CreditCardValidationResponse.new(fetched)
    else
