...
//     (\`new CreditCardValidationSDK(...)\` / \`CreditCardValidationSDK.test(...)\` become
//     \`CreditCardValidationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CreditCardValidationSDK'
...
const TEST_SEED = {"entity":{"validation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CreditCardValidationSDK(...)\` and any
// \`CreditCardValidationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CreditCardValidationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CreditCardValidationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
