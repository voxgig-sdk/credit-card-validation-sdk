# CreditCardValidation TypeScript SDK Reference

Complete API reference for the CreditCardValidation TypeScript SDK.
...
```ts
new CreditCardValidationSDK(options?: object)
```
...
#### `CreditCardValidationSDK.test(testopts?, sdkopts?)`
...
**Returns:** `CreditCardValidationSDK` instance in test mode.
...
Alias for `CreditCardValidationSDK.test()`.
...
```ts
const result = await client.Validation().load({ cc: 'cc' })
```
...
Return the parent `CreditCardValidationSDK` instance.
