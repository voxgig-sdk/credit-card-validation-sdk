...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("CREDIT_CARD_VALIDATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("CREDIT_CARD_VALIDATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("CREDIT_CARD_VALIDATION_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.CreditCardValidationSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.CreditCardValidationSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.CreditCardValidationError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.CreditCardValidationSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.CreditCardValidationError{Msg: msg, Code: code}
}
