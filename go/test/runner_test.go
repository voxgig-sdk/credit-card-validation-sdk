...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("CREDITCARDVALIDATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("CREDITCARDVALIDATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("CREDITCARDVALIDATION_TEST_EXPLAIN"); explain != "" {
