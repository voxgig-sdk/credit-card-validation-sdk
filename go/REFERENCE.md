# CreditCardValidation Golang SDK Reference

Complete API reference for the CreditCardValidation Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *CreditCardValidationSDK`
...
#### `Validation(data map[string]any) CreditCardValidationEntity`
...
```go
result, err := client.Validation(nil).Load(map[string]any{"cc": "cc"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
