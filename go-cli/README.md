# credit-card-validation-cli

boru-driven command-line client **and** interactive REPL for the CreditCardValidation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/credit-card-validation-cli)
make build

# 2. See usage (words, entities, env vars)
./credit-card-validation-cli --help

# 3. Provide credentials once, via the environment
export CREDIT_CARD_VALIDATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./credit-card-validation-cli load 1 validation            # {id:1} shorthand
./credit-card-validation-cli load '{id:1}' validation       # explicit match map

# 5. Override the API base URL for a single call
CREDIT_CARD_VALIDATION_BASE=https://api.example.com ./credit-card-validation-cli load 1 validation

# 6. No arguments -> interactive REPL
./credit-card-validation-cli
credit-card-validation> load 1 validation
credit-card-validation> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/credit-card-validation-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/credit-card-validation-cli load 1 validation
   ```
...
```sh
./credit-card-validation-cli load 1 validation          # scalar shorthand for {id:1}
./credit-card-validation-cli load '{id:1}' validation     # explicit match map
```
...
./credit-card-validation-cli load 1 validation
```
...
Run with no arguments to open a REPL (prompt `credit-card-validation>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `validation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/credit-card-validation-cli`. |
...
(`./credit-card-validation-cli <expr>`) and interactively (the REPL), and expressions compose the
