// Command CreditCardValidation-cli is a CLI + REPL for the CreditCardValidation SDK,
...
const prompt = "credit-card-validation"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "validation"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CREDIT_CARD_VALIDATION_APIKEY (api key), CREDIT_CARD_VALIDATION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CREDIT_CARD_VALIDATION_APIKEY carries the API key and
	// CREDIT_CARD_VALIDATION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CREDIT_CARD_VALIDATION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CREDIT_CARD_VALIDATION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCreditCardValidationSDK(opts)
