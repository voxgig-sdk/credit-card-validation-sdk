...
// CreditCardValidation SDK Validation entity
...
class ValidationEntity
{
    ...
    {
        if ($args) {
            $this->_data = CreditCardValidationHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = CreditCardValidationHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    /**
     * Load a single Validation.
     *
     * @param ValidationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ValidationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Validation|array The loaded Validation as an assoc-array at the
     *   SDK boundary; throws CreditCardValidationError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CreditCardValidationHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
