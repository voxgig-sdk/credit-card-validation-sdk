...
class ValidationEntityTest extends TestCase
{
    ...
    {
        $testsdk = CreditCardValidationSDK::test(null, null);
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CREDIT_CARD_VALIDATION_TEST_VALIDATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function validation_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("CREDIT_CARD_VALIDATION_TEST_VALIDATION_ENTID");
    ...
    if ($env["CREDIT_CARD_VALIDATION_TEST_LIVE"] === "TRUE") {
        $merged_opts = Vs::merge([
            [
            ],
            $extra ?? [],
        ]);
        $client = new CreditCardValidationSDK(Helpers::to_map($merged_opts));
    }
