...
// CreditCardValidation SDK feature test
//
...
class FtCtrl extends CreditCardValidationControl
{
...
class FtClient
{
    ...
    private CreditCardValidationUtility $utility;

    public function __construct(CreditCardValidationUtility $utility, array $options)
    {
    ...
    public function get_utility(): CreditCardValidationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CreditCardValidationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CreditCardValidationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CreditCardValidationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CreditCardValidationConfig::make_feature($name);
    ...
    private static function buildUrl(CreditCardValidationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CreditCardValidationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CreditCardValidationResponse($fetched);
        }
    ...
    private function fail(CreditCardValidationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CreditCardValidationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
