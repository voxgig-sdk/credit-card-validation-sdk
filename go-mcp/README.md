...
[MCP](https://modelcontextprotocol.io) server exposing the CreditCardValidation SDK as
two agent tools — `credit-card-validation_list` and `credit-card-validation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/credit-card-validation-mcp)
make build

# 2. Provide credentials via the environment
export CREDIT_CARD_VALIDATION_APIKEY=sk_live_xxx
...
```jsonc
// credit-card-validation_list: first page of records
{ "entity": "validation" }
{ "entity": "validation", "query": { } }

// credit-card-validation_load: one record by id
{ "entity": "validation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/credit-card-validation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user credit-card-validation \
     -- "$PWD"/dist/*/credit-card-validation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `credit-card-validation_list` and `credit-card-validation_load` tools now appear
   in new sessions. Ask the agent to *"list validation using credit-card-validation"*
   and it calls `credit-card-validation_list` with `{"entity":"validation"}`.
...
```sh
export CREDIT_CARD_VALIDATION_APIKEY=sk_live_xxx            # API key
export CREDIT_CARD_VALIDATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `credit-card-validation_list` tool
...
### Call the `credit-card-validation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `credit-card-validation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `credit-card-validation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CREDIT_CARD_VALIDATION_APIKEY` | API key sent with every request. |
| `CREDIT_CARD_VALIDATION_BASE` | Optional override of the API base URL. |
