# CreditCardValidation SDK test runner
...
class CreditCardValidationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CreditCardValidationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CreditCardValidationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CreditCardValidationTestRunner.getenv("CREDIT_CARD_VALIDATION_TEST_LIVE")
        override = CreditCardValidationTestRunner.getenv("CREDIT_CARD_VALIDATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CreditCardValidationTestRunner.getenv(key)
        ...
        explain = CreditCardValidationTestRunner.getenv("CREDIT_CARD_VALIDATION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if CreditCardValidationTestRunner._test_control is not None:
            return CreditCardValidationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CreditCardValidationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CreditCardValidationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CreditCardValidationTestRunner.load_test_control()
...
def load_env_local():
    CreditCardValidationTestRunner.load_env_local()


def env_override(m):
    return CreditCardValidationTestRunner.env_override(m)


def entity_data(v):
    return CreditCardValidationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CreditCardValidationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CreditCardValidationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CreditCardValidationTestRunner.load_test_control()


def live_delay_ms():
    return CreditCardValidationTestRunner.live_delay_ms()
